...
class AsyncSqlAlchemyOutboxStorageStrategy(AsyncOutboxStorageStrategy):
    """
    Async counterpart of `SqlAlchemyOutboxStorageStrategy`.

    Stores outbox entries in a relational table using an `AsyncSession`.
    """
    ...
    async def put_into_outbox(self, records: list[RecordedRaw]) -> None:
        rows = []
        ...
        await self._session.execute(insert(self._outbox_entry_model), rows)
