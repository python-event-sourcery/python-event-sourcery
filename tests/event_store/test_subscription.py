...
class TestHandlingUncommittedMessagesFromOtherClients:
    @pytest.mark.not_implemented(
        backend=["sqlalchemy_sqlite", "sqlalchemy_postgres"],
    )
...
@pytest.mark.skip_backend(backend="esdb", reason="ESDB don't have transactions")
class TestInTransactionSubscription:
    ...
    @pytest.fixture()
    def subscription(self, backend: Backend) -> Iterator[Subscription]:
