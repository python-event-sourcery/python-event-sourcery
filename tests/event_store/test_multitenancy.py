...
def test_streams_with_same_id_or_name_can_coexist(
    ...
    given.in_tenant_mode("tenant1").event(tenant_1_event := an_event(), on=stream_id)
    given.in_tenant_mode("tenant2").event(tenant_2_event := an_event(), on=stream_id)

    then.without_tenant().stream(with_id=stream_id).loads_only([without_tenant_event])
    then.in_tenant_mode("tenant1").stream(with_id=stream_id).loads_only(
        [tenant_1_event]
    )
    then.in_tenant_mode("tenant2").stream(with_id=stream_id).loads_only(
        [tenant_2_event]
    )
