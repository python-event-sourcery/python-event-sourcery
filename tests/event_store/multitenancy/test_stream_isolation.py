...
pytestmark = pytest.mark.skip_backend(
    backend=["esdb", "in_memory", "sqlalchemy_sqlite", "django"],
...
def test_stream_created_in_tenantless_context_cannot_be_accessed_from_tenant_context(
...
@pytest.mark.parametrize("stream_id", [StreamId(), StreamId(name="stream1")])
def test_stream_created_in_tenant_context_cannot_be_accessed_in_tenantless_context(
    event_store: EventStore, stream_id: StreamId
) -> None:
    tenant_1 = event_store.scoped_for_tenant("tenant1")
    tenant_1.append(TenantCreated(message="Hello 1!"), stream_id=stream_id)

    tenantless = event_store
    events = tenantless.load_stream(stream_id)
...
def test_streams_with_same_id_or_name_can_coexist(
    ...
    tenantless_stream = tenantless.load_stream(stream_id)
    assert len(tenantless_stream) == 1
    assert tenantless_stream[0].event == tenantless_event
